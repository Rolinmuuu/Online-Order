...
import { ApiError, checkout, clearCart, describeError, money, toCents } from "../api";
...
const MyCart = () => {
  ...
  const first = lines[0];
  const restaurant = first ? restaurants.find((r) => r.id === first.restaurant_id) : null;
        ...
        Modal.confirm({
          ...
          onOk: () =>
            checkout(newTotal)
              ...
              .catch((err) => message.error(describeError(err, "Could not place the order"))),
        });
      } else {
        message.error(describeError(e, "Could not place the order"));
      }
    } finally {
      setPlacing(false);
    }
