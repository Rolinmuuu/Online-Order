...
import { kitchenAction, kitchenBoard, kitchenRestaurants, money, subscribe } from "../api";
...
const KitchenPage = () => {
  ...
  const [board, setBoard] = useState({ orders: [], payable_cents: 0 });
  ...
  const load = useCallback(() => {
    ...
    kitchenBoard(rid)
      ...
      .catch(() => n === latest.current && message.error("Could not load the board"));
  }, [rid]);
      ...
      <div className="kitchen-head">
        <div>
          ...
          {restaurants.length > 1 && (
            <Segmented value={rid} onChange={setRid} options={restaurants.map((r) => ({ label: r.name, value: r.id }))} />
          )}
        </div>
                  ...
                  <div className="ticket-actions">
                    {col.actions.map(([action, label, kind]) =>
                      action === "reject" ? (
                        ...
                        <Button key={action} size="small" type={kind} loading={busy === `${o.id}:${action}`} onClick={() => act(o.id, action)}>
                          {label}
                        </Button>
                      )
                    )}
